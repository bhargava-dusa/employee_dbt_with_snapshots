{{ config(
    ...
    post_hook = ["
                    ...
                    set final_table_relation = adapter.get_relation(
                            ...
                            schema=this.schema,
                            identifier='transformed_employee'
                        )
                    %}
                    {#
                    ...
                    delete from {{ final_table_relation }} where {{ final_table_relation }}._airbyte_unique_key in (
                        ...
                        from (
                                ...
                                from {{ this }}
                                where 1=1 {{ incremental_clause('_airbyte_normalized_at', adapter.quote(this.schema) + '.' + adapter.quote('transformed_employee')) }}
                            ) recent_records
                            left join (
                                select _airbyte_unique_key as unique_key, count(_airbyte_unique_key) as active_count
                                from {{ this }}
                                where _airbyte_active_row = 1 {{ incremental_clause('_airbyte_normalized_at', adapter.quote(this.schema) + '.' + adapter.quote('transformed_employee')) }}
                    ...
                    {% endif %}
                    ","delete from _airbyte_transformations.transformed_employee_stg where _airbyte_emitted_at != (select max(_airbyte_emitted_at) from _airbyte_transformations.transformed_employee_stg)"],
    tags = [ "top-level" ]
) }}
-- depends_on: ref('transformed_employee_stg')
with
{% if is_incremental() %}
new_data as (
    ...
    select
        *
    from {{ ref('transformed_employee_stg')  }}
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('transformed_employee_stg'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('transformed_employee_stg')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('transformed_employee_stg')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
